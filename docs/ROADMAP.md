...
| **Le panneau d'assaut** | Quatre cadres imbriqués dans 300 px, et un schéma d'enceinte qui défilait. Une première tentative n'a réglé ni l'un ni l'autre : mesuré à trois tailles avec un champion en tête, le panneau défilait DÉJÀ replié (208 px de contenu pour 196 de plafond) et, déplié, recouvrait deux des trois jauges de secteur. La cause était structurelle - `translateY(-50%)` centre le panneau, donc sa hauteur utile vaut deux fois la plus courte distance au bord, soit 192 et non les 316 px de bande libre. Aucun `max-height` ne pouvait la lever. Il est désormais ANCRÉ sous la jauge du nord, ses sections sont des filets et non des boîtes, et l'ellipse de l'enceinte - 280 px, qui n'entrera jamais - laisse place sous les flèches à quatre rangs de 18 px : même composant, même geste, autre densité. Replié 305 × 159, déplié 305 × 239, aucune jauge recouverte. |
...
**Et une prémisse fausse, la mienne :** j'avais écrit aux agents que `DELAI_ORDRE_MS` limitait la
réassignation d'un pan. Vérification faite, il ne borne que la posture et le tir - `assignerSecteur`
ne lit jamais `prochainAt`. Réassigner est instantané et illimité, ce qui renverse l'arbitrage :
c'est le levier le plus réactif du joueur, pas le moins.
