...
/** dallage suggéré : pierres irrégulières en 2-3 tons posées sur un fond joint */
function Dallage({ rx, ry, seed = 5, marbre = false }: { rx: number; ry: number; seed?: number; marbre?: boolean }) {
  const rnd = alea(seed)
  const tons = marbre
    ? ['#ece5d2', '#e0d8c3', '#f2ecdb', '#d9d0ba']
    : ['#d3c9ae', '#c7bc9f', '#dcd3b9', '#cbc0a3']
...
/** autel du foyer public : bloc appareillé, corniche, cendres et feu perpétuel */
function Foyer({ n }: { n: number }) {
  const w = n >= 3 ? 14 : n === 2 ? 12 : 11
  return (
    <g transform="translate(0,-2)">
      {/* socle dallé rond dès que la place est pavée */}
...
/** poteau de bois de la stoa rustique — plus sombre que le mur pour se détacher */
function Poteau({ x, h }: { x: number; h: number }) {
  return (
    <g>
      <rect x={x - 1.6} y={-h} width={3.2} height={h} fill="#8d6f4c" />
      <line x1={x + 1.1} y1={-h + 0.5} x2={x + 1.1} y2={-0.5} stroke={PAL.boisOmbre} strokeWidth={1} />
      <line x1={x - 1.2} y1={-h + 0.5} x2={x - 1.2} y2={-0.5} stroke="#b08a5e" strokeWidth={0.8} />
      <rect x={x - 2.3} y={-h - 1.4} width={4.6} height={1.6} fill={PAL.boisOmbre} />
    </g>
  )
}
  ...
  return (
    <g>
      ...
      <g transform={`translate(0,${-podH})`}>
        ...
        <rect x={-w / 2 + 1.5} y={-h - 2} width={w - 3} height={h + 2} fill="url(#a-ao)" opacity={bois ? 0.45 : 0.75} />
        ...
        {bois &&
          [0.32, 0.58, 0.84].map((t) => (
            <line key={t} x1={-w / 2 + 2.5} y1={-h * t} x2={w / 2 - 2.5} y2={-h * t} stroke="#9c7f56" strokeWidth={0.8} opacity={0.55} />
          ))}
        {/* portes du fond */}
        {portes.map((px) => (
          <g key={px}>
...
function StatueArt({ x = 0, y = 0, or = false, s = 1 }: { x?: number; y?: number; or?: boolean; s?: number }) {
  ...
  return (
    <g transform={`translate(${x},${y})${s !== 1 ? ` scale(${s})` : ''}`}>
      ...
      {/* figure drapée, flanc droit dans l'ombre, bras levé tenant la lance */}
...
/** entrepôt du marché : petite bâtisse de pierre, jarres à demeure */
function Entrepot({ x, y, flip = false }: { x: number; y: number; flip?: boolean }) {
  return (
    <g transform={`translate(${x},${y})`}>
      <ellipse cx={0} cy={1.5} rx={17} ry={5} fill="#b9a878" opacity={0.45} />
      <Batisse3D w={24} h={11} g={6} prof={7} mat="pierre" toit="tuiles" enfants={<Porte3D w={6.6} h={8.6} x={flip ? 3 : -3} />} />
...
export function Agora({ n }: { n: number }) {
  if (n === 1) {
    return (
      <g>
        ...
        <Foyer n={1} />
        {/* natte de la marchande, marchandises posées au sol */}
        ...
        {/* banc : planche sur deux pierres */}
        <g transform="translate(-13,9)">
          <ellipse cx={1.5} cy={0.8} rx={7.5} ry={1.5} fill={PAL.ombrePortee} opacity={0.13} />
          <rect x={-5.5} y={-2.2} width={2.6} height={2.2} fill={PAL.pierreMi} />
          <rect x={3} y={-2.2} width={2.6} height={2.2} fill={PAL.pierreMi} />
          <rect x={-7} y={-3.6} width={14} height={1.6} fill="url(#a-bois-l)" />
          <rect x={-7} y={-3.6} width={14} height={0.5} fill="#c2a071" />
        </g>
        <OlivierMini x={30} y={-6} s={0.95} />
  ...
  return (
    <g>
      {/* place dallée : fond joint, dalles suggérées, rebord discret */}
      ...
      <ellipse cx={0} cy={0} rx={rx - 0.7} ry={ry - 0.5} fill="none" stroke={marbre ? '#a89d82' : '#96896b'} strokeWidth={1} opacity={0.45} />
      {/* cercle d'or incrusté dans le dallage, autour du foyer */}
      {marbre && (
        <g>
          <ellipse cx={0.4} cy={0.4} rx={26} ry={7.8} fill="none" stroke="#8a6b2e" strokeWidth={0.9} opacity={0.55} />
          <ellipse cx={0} cy={0} rx={26} ry={7.8} fill="none" stroke={PAL.or} strokeWidth={0.9} opacity={0.85} />
          {[[-26, 0], [26, 0]].map(([gx, gy]) => (
            <circle key={`${gx},${gy}`} cx={gx} cy={gy} r={1} fill={PAL.or} opacity={0.9} />
          ))}
        </g>
      )}
      ...
      {n >= 3 && <EtalArt x={0} y={14} c="#7c6a9c" biens="fruits" s={0.88} />}

      {/* statue civique bien dégagée, à l'avant de la place */}
      {n >= 3 && <StatueArt x={marbre ? 45 : 41} y={marbre ? 9 : 8} or={marbre} s={1.15} />}

      {/* fret en attente près des étals */}
      <Corbeille x={-18} y={9} s={0.95} fruits="#d9b25a" />
      <Amphore x={-15} y={10.5} s={0.8} />
      {n >= 3 && <Amphore x={20} y={-1} c="#8c552f" s={0.75} />}
