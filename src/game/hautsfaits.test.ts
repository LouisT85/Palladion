...
function regneTotal(modif: Partial<SnapHautFait> = {}): SnapHautFait {
  ...
  return village({
    ...
    faveur: 100,
    army: { lancier: 40, archer: 40, hoplite: 40 },
...
describe('les conditions', () => {
  ...
  it('respecte au chiffre près les seuils annoncés au joueur', () => {
    ...
    const paliers: [string, Partial<SnapHautFait>, Partial<SnapHautFait>][] = [
      ...
      ['petite-armee', { army: { lancier: 7, archer: 7, hoplite: 5 } }, { army: { lancier: 7, archer: 7, hoplite: 6 } }],
      ['phalange', { army: { lancier: 40, archer: 40, hoplite: 7 } }, { army: { lancier: 0, archer: 0, hoplite: 8 } }],
...
describe('la couture avec le store', () => {
  ...
  it('recopie fidèlement la partie dans la vue que jugent les hauts faits', () => {
    useGame.setState((s) => {
      ...
      s.tours = 3
      s.faveur = 66
      s.army = { lancier: 5, archer: 3, hoplite: 12 }
    ...
    expect(vu.army).toEqual({ lancier: 5, archer: 3, hoplite: 12 })
